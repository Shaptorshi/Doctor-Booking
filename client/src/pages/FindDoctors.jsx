...
import { Search, Filter, MapPin, Calendar, Star,DollarSign } from "lucide-react";
        ...
        <div className="doctor-grid">
          {search.length > 0 ? search.map((item, index) => (
            <div key={index} className="doctor-card">
              ...
              <p className="fees"><DollarSign size={20}/>{item.fees}</p>
              <button className="appointment-btn">
                <Link style={{ textDecoration: "none" }} to={"/finalAppointments" + "?id=" + `${item.id}`}>
                  <Calendar size={16} />
                  Book Appointment
                </Link>
              </button>
